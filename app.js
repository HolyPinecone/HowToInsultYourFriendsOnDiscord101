...
client.on('messageCreate', (msg) => {
    ...
    if (content.includes('crazy')) {
        msg.channel.send('Crazy? I was crazy once. They locked me in a room. A rubber room. A rubber room filled with rats and the rats made me crazy.');
        ...
        msg.channel.send('quiet, go back to sleep. U still seem to be too tired to write u lazy fuck.');
    } if (content.includes('night')) {
        msg.channel.send('sleep is for the weak pussy');
    }if (content === ('gn')) {
        msg.channel.send('No need to wake up until you start writing properly you lazy fuck.');
...
(async ()=>{
    ...
    await client.rest.put(Routes.applicationCommands(client.user.id), { body: Object.values(commands).map(ch => ch.definition) });
